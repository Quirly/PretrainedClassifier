...
def classify_images(images_dir, results_dic, model):
    ...
    #return None
    #results_dic=get_pet_labels.py
     
    # The full path with image name is created in each iteration cycle
    # Returned string ist processed, whitespaces at the end are deleted
    # and all characters are formated to lower letters so that comparison
    # will be possible
    # the 'cleaned' classifier string is then added to results_dic values
    # as a second element in the value list
    # Pet label and Classifier label are then compared
    # If there is a match, a numeric value of 1 will be added to the value
    # list of the dicitionary - otherwise 0 is is added (value list index = 3)
    for key,pet_image_label in results_dic.items():
        test_image=str(images_dir)+str(key)
        model_label_raw = classifier(test_image, model)
        #TODO 3B
        #model_label = model_label_raw.lower().replace(", ","").strip()
        model_label = model_label_raw.lower().rstrip()
        truth=str(pet_image_label)
        #Modified after project review on 05/10/2020 by Susanne:
        #pet_image_label.append(model_label)
        if results_dic[key][0] in model_label:
        #if results_dic[key][0] in results_dic[key][1]:
            results_dic[key].extend([model_label,1])
            #pet_image_label.append(0)
        else:
    ...
    #return results
     # TODO: 3b. BELOW REPLACE pass with CODE to process the model_label to 
       ...
       # results_dic as an exact match(1) using extend list function
            ...
            #mylist=list(results_dic[value])
            #mylist.extend(model_label)
           # mylist.extend(1)
